...
class Logger:
    ...
    logging.basicConfig(
        ...
        datefmt='%H:%M:%S',
        handlers=[
        logging.FileHandler(join("logs","log_{0}_{1}".format(__name__,time.strftime("%Y-%m-%d", time.gmtime())))),
...
class Decorators:
    ...
    def run_time(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            log.info(f'Execution Time (hh:mm:sec) for function "{func.__name__}": {h:02d}:{m:02d}:{s:02d},{ms:03d}')
            return ret
        return wrapper
                ...
                while t <= times:
                    try:
                        ...
                        if t <= times:
                            ...
                            log.error(f'Function "{func.__name__}" could not be executed after {times} tries')
            return wrapper
        return decorator
            ...
            def wrapper(*args, **kwargs):
                ...
                while t <= times:
                    try:
                        ...
                        t += 1
                        if t <= times:
                            log.info(f'Stalling {round(delay, 3)} secs before next execution try.')
                            time.sleep(delay)
                            delay *= 2
                        else:
                            log.error(f'Function "{func.__name__}" could not be executed after {times} tries')
    ...
    @staticmethod
    def accepted_arguments_within_class_instances(accepted_args):
        '''
        When decorating a function with this decorator, the function's arguments are checked against a list of valid arguments.
...
class Date_Filterer:
    ...
    def get_unique_write_days(self):
        self.unique_write_days = sorted(list(set([self.extract_date_from_name(file) for file in self.items if self.extract_date_from_name(file)])))
        log.info(f'The following unique days of data writes have been found {self.unique_write_days}')
        return self
    ...
    def get_days_to_delete(self):
        if self.unique_write_days:
            ...
            self.days_to_delete = [day for day in older_writes if (self.as_date_time(newest_writes) - self.as_date_time(day)).days >= self.delta_days]
            return self

    def get_files_to_delete(self):
        files_to_delete = [file for file in self.items if any([file.startswith(day) for day in self.days_to_delete])]
        if files_to_delete:
            log.info(f'The following files are about to be deleted as they are {self.delta_days} days apart from the recent writes:\n {files_to_delete}')
        else:
            log.info(f'No files were found that would have been deleted as they are {self.delta_days} days apart from the recent writes.')
...
class SubstringFinder(Logger):
    ...
    '''
    # .encode('unicode-escape').decode()  --> convert to raw strings
...
class Project_Initializer(Logger):
    ...
    def __init__(self,
        ...
        self.log.info('''You just initiliazed a Machine Learning project creater instance. Usage is as follows:
    ...
    def create_docker_file(self):
        ...
        ENTRYPOINT ["python"] 
        CMD ["src/{3}" ]
        '''.format(self.python_version, self.user, self.e_mail, self.main_file)
        with open('Dockerfile', 'w') as docker:
            docker.write(docker_cmd)

    def create_requirements(self,
                            standard_packages=['pandas==0.24.2',
                                               'numpy==1.16.2',
                                               'seaborn',
                                               'matplotlib==3.0.3',
                                               'scikit-learn==0.20.3',
                                               'jupyterlab==0.35.4'],  # sphinx
                            add_packages=[]):
...
cmd /K
        '''.format(conda_activate, env_name)
        with open('setup_conda_env.bat', 'w') as env_setup:
            env_setup.write(env_cmd)
        self.log.info('Created "setup_conda_env.bat" batch file for generating conda environment')
        if jupyter:
            try:
                assert jupyter in ['lab', 'notebook']
            except AssertionError:
                self.log.error('Jupyter kernels can only be called via "lab" or "notebook". Please use one of them.')
            self.create_Jupyter_launcher(conda_activate,jupyter)

    def create_Jupyter_launcher(self, conda_activate,jupyter_instance):
        self.log.info('Please ensure that Jupyter lab/notebook is defined in the requirements.txt file')
        jupyter_cmd = '''
call {0}\\Scripts\\activate.bat
call conda activate {1}
call jupyter {2}
        '''.format(conda_activate, self.conda_env_name, jupyter_instance)
        with open('launch_jupyter.bat', 'w') as jupyter:
            jupyter.write(jupyter_cmd)
        self.log.info('Created "launch_jupyter.bat" batch file for launching a Jupyter lab')
...
Contact Information = {2}
        '''.format(self.python_version, self.user, self.e_mail)
